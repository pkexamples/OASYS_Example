...
namespace PhotonKinetics.OASYS
{
    ...
    public class AnalysisDatabase
    {
        ...
        public AnalysisDatabase(string udlFile)
        {
            ...
            {
                this.connectionString = "File Name=" + udlFile;
            }
        }
        ...
        public AnalysisSetupType AnalysisSetup(int databaseID)
        {
                ...
                using (var command = new OleDbCommand(cmd, conn))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            this.ReadAnalysisParams(setup, wavelengthKey, gi);
                            this.ReadSlidingWindowParams(setup, wavelengthKey, gi);
                            setup.Limits = new LimitSetType[2];
                            this.ReadLimitSet(setup, wavelengthKey, gi, DirectionalityAspectType.Unidirectional);
                            this.ReadLimitSet(setup, wavelengthKey, gi, DirectionalityAspectType.Bidirectional);
                            setups.Add(setup);
                        }
                    }
                }
        ...
        {
            int analysisID = this.DatabaseID(limitSet);
            ...
            using (var conn = new OleDbConnection(this.connectionString))
            {
                ...
                this.DeleteSpectral(conn, analysisID);
                using (var command = new OleDbCommand())
                {
                    ...
                    {
                        this.DeleteLimits(conn, limitID);
                    }
        ...
        private void DeleteSpectral(OleDbConnection conn, int analysisID)
        {
            ...
            using (var command = new OleDbCommand())
            {
                ...
                if (limitsID > -1)
                {
                    this.DeleteLimits(conn, limitsID);
                }
            }
        }
        ...
        private void ReadSlidingWindowParams(AnalysisWavelengthSetupType setup, int wavelengthKey, double gi)
        {
            using (var conn = new OleDbConnection(this.connectionString))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        setup.AnalysisParams.WindowStep = this.SecToMeters((double)reader["windowStep"], gi);
                        setup.AnalysisParams.WindowWidth = this.SecToMeters((double)reader["windowWidth"], gi);
                    }
                    else
                    {
            ...
            using (var conn = new OleDbConnection(this.connectionString))
            {
                ...
                using (var cmd = new OleDbCommand(qry, conn))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        if (reader.Read())
                        {
                            setup.AnalysisParams.MinAttenuation = 1000.0D / this.SecToMeters(1 / (double)reader["minAtten"], gi);
                            setup.AnalysisParams.MaxAttenuation = 1000.0D / this.SecToMeters(1 / (double)reader["maxAtten"], gi);
                            setup.AnalysisParams.Sensitivity = (double)reader["sensitivity"];
                            setup.AnalysisParams.EndLossThresh = (double)reader["endLossThresh"];
                            setup.AnalysisParams.WindowStartOffset = this.SecToMeters((double)reader["bottomEventGuardLen"], gi);
                            setup.AnalysisParams.WindowStopOffset = this.SecToMeters((double)reader["topEventGuardLen"], gi);
                        }
                        else
                        {
            ...
            using (var conn = new OleDbConnection(this.connectionString))
            {
                ...
                using (var cmd = new OleDbCommand(qry, conn))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        {
                            int key;
                            double scale = this.SecToMeters(1, gi);
                            ...
                            if (directionality == (int)DirectionalityAspectType.Unidirectional)
                            {
                                ...
                                limitSet.Attenuation = this.ReadLimits(key, 1000.0 / scale);
                                key = (int)reader["uniEventLoss"];
                                limitSet.EventLoss = this.ReadLimits(key);
                            }
                            else
                            {
                                key = (int)reader["biAttenCoef"];
                                limitSet.Attenuation = this.ReadLimits(key, 1000.0D / scale);
                                key = (int)reader["biEventLoss"];
                                limitSet.EventLoss = this.ReadLimits(key);
                            }

                            key = (int)reader["bufferLoss"];
                            limitSet.BufferLoss = this.ReadLimits(key);
                            key = (int)reader["bufferRefl"];
                            limitSet.BufferReflectance = this.ReadLimits(key);
                            key = (int)reader["eventRefl"];
                            limitSet.EventReflectance = this.ReadLimits(key);
                            key = (int)reader["fiberLength"];
                            limitSet.FiberLength = this.ReadLimits(key);
                            key = (int)reader["lsaDeviation"];
                            limitSet.LsaPointDeviation = this.ReadLimits(key);
                            key = (int)reader["slidingWindowAtten"];
                            limitSet.WindowAttenuation = this.ReadLimits(key, 1000.0D / scale);
                            key = (int)reader["slidingWindowUni"];
                            limitSet.WindowUniformity = this.ReadLimits(key, 1000.0D / scale);
                        }
                        else
                        {
        ...
        {
            return 0.5 * this.speedOfLight * t / gi;
        }
        #endregion
    }
}
